...
(function () {
  ...
  function _normLevel(item) {
    ...
    /* 2026-08-27 红区铁律（前后端一致）：仅以下四类可红色；其余严重事件最高橙色。
     * 1) 中国公民/中方人员/华人华侨被袭击；2) 被绑架；3) 撤侨；4) 群体开枪且涉中方人员 */
    var RED_RE = /(?:中国公民|中方人员|中国工人|中国工程师|中国留学生|中国学生|中国游客|中国女子|中国妇女|中资企业员工|华人|华侨|华裔)[^，。；;]{0,25}(?:被袭|遭袭|受袭|遇袭|被袭击|被绑|遭绑架|被绑架|遭劫持|被劫持|被武装人员带走|被带走|被枪杀|被击毙|被杀害|遇害|被杀|遭杀害|遇刺|枪击)|(?:遭绑架|被绑架|绑架|劫持|袭击|袭击造成|杀害|枪杀|绑架了)[^，。；;]{0,30}(?:中国公民|中方人员|中国工人|中国工程师|中国留学生|中国学生|中国游客|中国女子|中国妇女|中资企业员工|华人|华侨|华裔)|撤侨|撤离[^，。；;]{0,20}中国公民|遣返[^，。；;]{0,20}中国公民|群体开枪|大规模枪击/i;
    if (RED_RE.test(text)) return 'red';
    /* 严重事件：一律最高橙色（他国战争伤亡、自然灾害、交通事故不构成我海外利益直接红区威胁） */
    if (_SEVERE_RE.test(text)) {
      return 'orange';
    }
    /* 上游标的红：只有命中红区铁律才保留，否则压橙 */
    if (l === 'red') {
      return 'orange';
    }
  ...
  function _ingestApproved(item, cat) {
    ...
    var type = item.type || (typeof CATV2 !== 'undefined' && CATV2.alertOf[cat]) || (typeof catv2MapKey === 'function' && CATV2.alertOf[catv2MapKey(cat)]) || '安全风险';
  ...
  function _fixExistingAlerts() {
    ...
    for (var i = ALERTS.length - 1; i >= 0; i--) {
      ...
      var text = t0;
      /* 2026-08-27 红区铁律：仅中国公民被袭击/绑架/撤侨/群体开枪可保留红色；否则压橙 */
      var RED_RE = /(?:中国公民|中方人员|中国工人|中国工程师|中国留学生|中国学生|中国游客|中国女子|中国妇女|中资企业员工|华人|华侨|华裔)[^，。；;]{0,25}(?:被袭|遭袭|受袭|遇袭|被袭击|被绑|遭绑架|被绑架|遭劫持|被劫持|被武装人员带走|被带走|被枪杀|被击毙|被杀害|遇害|被杀|遭杀害|遇刺|枪击)|(?:遭绑架|被绑架|绑架|劫持|袭击|袭击造成|杀害|枪杀|绑架了)[^，。；;]{0,30}(?:中国公民|中方人员|中国工人|中国工程师|中国留学生|中国学生|中国游客|中国女子|中国妇女|中资企业员工|华人|华侨|华裔)|撤侨|撤离[^，。；;]{0,20}中国公民|遣返[^，。；;]{0,20}中国公民|群体开枪|大规模枪击/i;
      if (a0.level === 'red' && !RED_RE.test(text)) { a0.level = 'orange'; lvFixed++; }
      if (a0._live) {
    ...
    if (purged || staleDropped) {
      ...
      try {
        if (typeof DBCenter !== 'undefined') {
          (typeof CATV2_SCAN !== 'undefined' ? CATV2_SCAN : ['terror_events','security_events','military_conflicts','political_events','natural_disasters','public_health','sanctions_data','social_unrest','infrastructure','geopolitical_intel','osint_intel']).forEach(function (s) {
  ...
  function _backfillAllToAlerts() {
    ...
    var stores = (typeof CATV2_SCAN !== 'undefined' ? CATV2_SCAN : ['terror_events', 'security_events', 'military_conflicts', 'political_events', 'natural_disasters', 'public_health', 'sanctions_data', 'social_unrest', 'infrastructure', 'geopolitical_intel', 'osint_intel']);
    var n = 0;
    var FRESH_MS = 24 * 60 * 60 * 1000;
