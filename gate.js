...
(function (root) {
  ...
  var AK_ROUTINE_ENFORCEMENT_RE = /\b(?:bailiff|bailiffs|debt|debtor|creditor|auction|unpaid|arrears|owes?|owing|loan default|repossess|foreclos|evict|tax lien|revenue office|debt recovery|debt collection|强制执行|清债|讨债|债务|欠费|欠税|拍卖|查封办公室|查封资产|扣押财产|扣押资产|法警|执行局|债主|债权人|债务人|贷款违约|拖欠|欠款|欠缴|强制腾退|强行带走)\b/i;
  var AK_ROUTINE_ENFORCEMENT_ACTION_RE = /\b(?:storm|storms|stormed|raided|raid|seize|seized|seizure|seizing|confiscat|attach|attached|take over|took over|taken over|repossess|repossessed|evict|evicted|foreclos|foreclosed|强制进入|冲入|闯入|占领|查封|扣押|强制腾退)\b/i;
...
var hasStrongOverseas = AK_STRONG_OVERSEAS_RE.test(text);
      ...
      var hasRisk = _akChinaNegative(text) || AK_TOPIC_SECURITY_RE.test(text) ||
        /遭袭|遇害|遇难|伤亡|死伤|制裁|出口管制|技术封锁|禁令|禁止|冻结|扣押|查封|调查|审查|抗议|示威|冲突|争端|摩擦|紧张|风险|威胁|警告|指责|批评|谴责|限制|打压|排斥|歧视|侮辱|辱骂|骚扰|恐吓|反华|排华|敌意|负面|危机|纠纷|诉讼|仲裁|罚款|处罚|违约|亏损|破产|倒闭|裁员|停工|停产|停建|中断|撤离|疏散|炮击|逼近|袭击|攻击|恐袭|爆炸|枪击|绑架|劫持|人质|谋杀|刺杀|暴力|抢劫|盗窃|破坏|损毁|损失|死|亡|伤|危|险|擅闯|闯入|入侵|非法进入|围殴|殴打|伤害/i.test(text);
