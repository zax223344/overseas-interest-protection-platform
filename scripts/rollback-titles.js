/**
 * 回滚被错误模板化补全的标题：恢复到 title_en（原始英文标题）
 */
const { query } = require('C:/Users/28737/Desktop/新建文件夹/server/db.js');

const POLLUTED_RE = /海外安全事件引关注|发生海外安全事件|海外发生.*事件引关注/;

async function rollback() {
  const { rows } = await query(
    `SELECT id, title, data_json FROM intel_data WHERE data_json->>'title_zh' ~ $1`,
    ['海外安全事件引关注|发生海外安全事件|海外发生.*事件引关注']
  );

  console.log(`发现 ${rows.length} 条被模板污染的记录`);
  let restored = 0;
  ...
  for (const row of rows) {
    ...
    restored++;
    console.log(`[RESTORED] id=${row.id}: ${row.title} -> ${original}`);
  }
