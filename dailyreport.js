...
/* 情报类型中文名（客户端版；v2.0 18 子类 + 旧 key 兼容，与服务端 _DR_TYPE_NAMES 同步） */
var _DR_TYPE_NAMES_C = (function () {
  var m = {
    ...
    /* v2.0 新 key */
    mass_violence: '群体性暴力', crime_events: '社会治安事件', regime_change: '政权变动',
    election_events: '选举事件', policy_shift: '政策法规突变', financial_market: '金融市场风险',
    business_climate: '营商环境恶化', industrial_accident: '生产安全事故', environmental_event: '环境生态事件'
  };
  return m;
})();
var DAILY_REPORT = {
  ...
  saveEdits: function () {
    ...
    var _body = { items: payload.items, sections: payload.sections, note: '人工编辑（改序/删除/改标题摘要）' };
    var _done = function () {
    ...
    var _fail = function (msg) {
      if (/缺少签名头|签名校验失败|签名时间戳/.test(msg || '')) msg += '（请退出登录后重新登录）';
      showToast('⚠️ 保存失败：' + (msg || '未知错误'));
    };
    /* 服务端挂 _signCheck：走 APIClient 自动注入签名头（裸 fetch 必 401，2026-09-04 修复） */
    if (typeof APIClient !== 'undefined' && APIClient._fetch) {
      APIClient._fetch('PUT', '/api/reports/daily/' + this._current, _body)
        .then(function (j) { if (j && j.ok) _done(); else _fail((j && j.error)); })
        .catch(function (e) { _fail(e.message); });
    } else {
      fetch('/api/reports/daily/' + this._current, {
        ...
        if (d.ok && d.j.ok) _done(); else _fail(d.j && d.j.error);
      }).catch(function (e) { _fail(e.message); });
    }
  },
  ...
  openDrawer: function (idx) {
    ...
    var h =
      ...
      + '<span class="dr-tag">类型：' + esc(_DR_TYPE_NAMES_C[it.type] || it.type || '—') + '</span>'
