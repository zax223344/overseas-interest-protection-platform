...
var WORKBENCH = {
  ...
  _toolbarHTML: function () {
    ...
    /* 公文输出（#569：按当前时间窗 v4 五产物生成《情报会商纪要》） */
    html += '<span class="dc-tab" id="wb-govdoc" style="cursor:pointer;font-size:11px" title="基于当前时间窗的指数/环比/异动/盯防/项目威胁数据，生成公文《情报会商纪要》">📄 公文输出</span>';
    /* 满宽地图 */
  ...
  _watchHTML: function () {
    ...
    var anom = this._anomList || [];
    var hN = this._hours === 168 ? '7 天' : this._hours + ' 小时';
    var html = '<div class="card-tt"><span class="ic">🎯</span>重点盯防 Top5<span style="margin-left:auto;font-weight:400;font-size:11px;color:var(--text3)">量×级别×涉华×项目×异动 · ' + hN + ' · 点击开复合详情</span></div>';
  ...
  _threatHTML: function () {
    ...
    var hN = this._hours === 168 ? '7 天' : this._hours + ' 小时';
    var html = '<div class="card-tt"><span class="ic">🏗</span>受威胁项目联动<span style="margin-left:auto;font-weight:400;font-size:11px;color:var(--text3)">项目档案×所在国预警 · ' + hN + ' · 点击开复合详情</span></div>';
    ...
    rows.forEach(function (r) {
      ...
      html += '<div class="wb-threat" data-wc="' + self._esc(r.country) + '" data-proj="' + self._esc(r.proj) + '">' +
  ...
  /* 异动行点击 → 拉地图到该国；盯防/项目行点击 → 复合详情抽屉（2026-09-05 用户指令） */
  _bindAnom: function () {
    var self = this;
  ...
  _bindWatch: function () {
    var self = this;
    Array.prototype.forEach.call(document.querySelectorAll('.wb-watch[data-wc]'), function (el) {
      el.onclick = function () { self._openCountryDrawer(el.getAttribute('data-wc')); };
    });
    Array.prototype.forEach.call(document.querySelectorAll('.wb-threat[data-wc]'), function (el) {
      el.onclick = function () { self._openProjectDrawer(el.getAttribute('data-proj'), el.getAttribute('data-wc')); };
    });
  },
  ...
  /* ═══════════ 复合详情抽屉（2026-09-05 用户指令一） ═══════════
   * 盯防国家 / 受威胁项目 → 右侧抽屉：要素卡 + 级别分布 + 24h 时间线 +
   * 关联预警（点击进全局预警详情）+ 关联项目 + 处置动作（地图定位/专项作战/盯防要点）。 */
  _closeDrawer: function () {
    var m = document.getElementById('wb-drawer-mask'); if (m) m.remove();
    var d = document.getElementById('wb-drawer'); if (d) d.remove();
  },
  _drawerShell: function (titleHtml, actHtml, bodyHtml) {
    this._closeDrawer();
    var mask = document.createElement('div');
    mask.className = 'wb-drawer-mask'; mask.id = 'wb-drawer-mask';
    mask.onclick = function () { WORKBENCH._closeDrawer(); };
    var box = document.createElement('div');
    box.className = 'wb-drawer'; box.id = 'wb-drawer';
    box.innerHTML = '<div class="wb-dh"><div style="display:flex;align-items:flex-start;gap:10px">' +
      '<div style="flex:1;min-width:0">' + titleHtml + '</div>' +
      '<button class="wb-mx" style="font-size:20px" onclick="WORKBENCH._closeDrawer()">×</button></div></div>' +
      (actHtml ? '<div class="wb-dact">' + actHtml + '</div>' : '') +
      bodyHtml;
    document.body.appendChild(mask);
    document.body.appendChild(box);
  },
  /* 时间窗内该国全部预警（新→旧） */
  _countryAlerts: function (country) {
    var now = Date.now(), win = this._hours * 3600 * 1000;
    var out = [];
    this._alerts().forEach(function (a) {
      if (!a || a.country !== country) return;
      var t = WORKBENCH._tsOf(a);
      if (!t || now - t > win) return;
      out.push(a);
    });
    out.sort(function (x, y) { return WORKBENCH._tsOf(y) - WORKBENCH._tsOf(x); });
    return out;
  },
  /* 24h 逐小时迷你时间线（7 天窗口按天聚合） */
  _timelineHTML: function (alerts) {
    var now = Date.now();
    var buckets = {}, labels = [], n = this._hours === 168 ? 7 : 24;
    var stepMs = this._hours === 168 ? 24 * 3600 * 1000 : 3600 * 1000;
    for (var i = n - 1; i >= 0; i--) {
      var k = Math.floor((now - i * stepMs) / stepMs);
      buckets[k] = 0;
      var d = new Date(k * stepMs);
      labels.push({ k: k, lab: this._hours === 168 ? (d.getMonth() + 1) + '/' + d.getDate() : d.getHours() + '时' });
    }
    (alerts || []).forEach(function (a) {
      var k = Math.floor(WORKBENCH._tsOf(a) / stepMs);
      if (buckets[k] != null) buckets[k]++;
    });
    var max = Math.max.apply(null, labels.map(function (l) { return buckets[l.k]; }).concat([1]));
    var html = '<div class="wb-tl">';
    labels.forEach(function (l) {
      var v = buckets[l.k];
      var h = Math.max(3, Math.round(v / max * 48));
      html += '<span class="wb-tlb" style="height:' + h + 'px' + (v >= max * 0.66 && v > 0 ? ';background:linear-gradient(180deg,var(--red),rgba(239,68,68,.35))' : v > 0 ? '' : ';opacity:.18') + '" title="' + l.lab + '：' + v + ' 条"></span>';
    });
    html += '</div><div style="display:flex;justify-content:space-between;font-size:9.5px;color:var(--text3)"><span>' + labels[0].lab + '</span><span>' + (this._hours === 168 ? '按天聚合' : '逐小时') + ' · 峰值 ' + max + ' 条</span><span>' + labels[labels.length - 1].lab + '</span></div>';
    return html;
  },
  /* 该国建档项目（ENTERPRISES 真实档案） */
  _countryProjects: function (country) {
    var out = [];
    this._enterprises().forEach(function (ent) {
      (ent.projects || []).forEach(function (p) {
        if (String(p.c || '') === country) out.push({ n: p.n, ent: ent.short || ent.name || '', inv: p.inv || '', p: p.p || '' });
      });
    });
    return out;
  },
  _lvColor: function (lv) { return lv === 'red' ? 'var(--red)' : lv === 'orange' ? 'var(--orange)' : lv === 'yellow' ? 'var(--yellow)' : 'var(--text3)'; },
  _lvCN: function (lv) { return lv === 'red' ? '红' : lv === 'orange' ? '橙' : lv === 'yellow' ? '黄' : '蓝'; },
  _fmtT: function (a) { var t = this._tsOf(a); return t ? new Date(t).toLocaleString('sv-SE').slice(5, 16) : '—'; },

  /* ── 盯防国家复合抽屉 ── */
  _openCountryDrawer: function (country) {
    if (!country) return;
    var self = this;
    var w = null;
    (this._watch || []).forEach(function (r) { if (r.country === country) w = r; });
    var alerts = this._countryAlerts(country);
    var projs = this._countryProjects(country);
    var hN = this._hours === 168 ? '7 天' : this._hours + ' 小时';
    /* 头部：国名 + 要素 chips */
    var title = '<div class="wb-dtitle">🎯 ' + this._esc(country) + '<span style="font-size:11px;font-weight:400;color:var(--text3)">重点盯防复合详情 · 近 ' + hN + '</span></div>' +
      '<div class="wb-dchips">' +
      '<span class="wb-chip">预警 <b style="color:var(--text)">' + alerts.length + '</b></span>' +
      (w && w.red ? '<span class="wb-chip" style="border-color:var(--red)55;color:var(--red)">红 <b>' + w.red + '</b></span>' : '') +
      (w && w.orange ? '<span class="wb-chip" style="border-color:var(--orange)55;color:var(--orange)">橙 <b>' + w.orange + '</b></span>' : '') +
      (w && w.yellow ? '<span class="wb-chip" style="border-color:var(--yellow)55;color:var(--yellow)">黄 <b>' + w.yellow + '</b></span>' : '') +
      (w && w.cn ? '<span class="wb-chip" style="border-color:var(--orange)55;color:var(--orange)">涉华 <b>' + w.cn + '</b></span>' : '') +
      (w && w.core ? '<span class="wb-chip" style="border-color:var(--red)55;color:var(--red)">核心 <b>' + w.core + '</b></span>' : '') +
      (projs.length ? '<span class="wb-chip" style="border-color:var(--cyan)55;color:var(--cyan)">建档项目 <b>' + projs.length + '</b></span>' : '') +
      (w && w.anom ? '<span class="wb-chip" style="border-color:var(--orange)55;color:var(--orange)">⚠ 速率超基线 <b>' + w.anom.ratio + '×</b></span>' : '') +
      (w ? '<span class="wb-chip">盯防分值 <b style="color:var(--orange)">' + w.score + '</b></span>' : '') +
      '</div>';
    /* 动作条 */
    var act = '<button class="wb-btn pri" onclick="WORKBENCH._closeDrawer();WORKBENCH._flyToCountry(\'' + this._esc(country).replace(/'/g, '') + '\')">🗺 地图定位</button>' +
      '<button class="wb-btn" onclick="WORKBENCH._gotoThreatroom(\'' + this._esc(country).replace(/'/g, '') + '\')">🎯 转入专项作战</button>' +
      '<button class="wb-btn" onclick="WORKBENCH._copyWatchNote(\'' + this._esc(country).replace(/'/g, '') + '\')">📋 复制盯防要点</button>';
    /* 主体 */
    var body = '<div class="wb-dsec"><div class="wb-dst">预警时间线（' + hN + '）</div>' + this._timelineHTML(alerts) + '</div>';
    if (projs.length) {
      body += '<div class="wb-dsec"><div class="wb-dst">关联中资项目（' + projs.length + '）</div>';
      projs.forEach(function (p) {
        body += '<div class="wb-pitem" onclick="WORKBENCH._openProjectDrawer(\'' + self._esc(p.n).replace(/'/g, '') + '\',\'' + self._esc(country).replace(/'/g, '') + '\')">' +
          '<span style="width:9px;height:9px;background:var(--cyan);transform:rotate(45deg);flex-shrink:0;box-shadow:0 0 6px #00d4ff66"></span>' +
          '<span style="font-weight:700;color:var(--text)">' + self._esc(p.n) + '</span>' +
          '<span style="color:var(--text3);font-size:11px">' + self._esc(p.ent) + (p.inv ? ' · 投资' + self._esc(p.inv) + '亿' : '') + (p.p ? ' · ' + p.p + '人' : '') + '</span>' +
          '<span style="margin-left:auto;font-size:10.5px;color:var(--cyan)">详情 →</span></div>';
      });
      body += '</div>';
    }
    body += '<div class="wb-dsec"><div class="wb-dst">关联预警（' + alerts.length + ' 条 · 点击查看全局预警详情）</div>';
    if (!alerts.length) body += '<div style="font-size:11.5px;color:var(--text3);padding:6px 0">时间窗内该国无预警</div>';
    alerts.slice(0, 15).forEach(function (a) {
      var lc = self._lvColor(a.level);
      body += '<div class="wb-aitem" style="border-left-color:' + lc + '" onclick="WORKBENCH._openAlert(\'' + String(a.id || '').replace(/'/g, '') + '\')">' +
        '<div class="t">' + self._esc(a.title_zh || a.title || '（无标题）') + '</div>' +
        '<div class="m"><span>' + self._esc(a.source || '—') + '</span><span>' + self._fmtT(a) + '</span>' +
        (a.level ? '<span style="color:' + lc + ';font-weight:700">' + self._lvCN(a.level) + '级</span>' : '') +
        (a.chinaRelated ? '<span style="color:var(--orange)">涉华</span>' : '') +
        (a.is_core ? '<span style="color:var(--red)">★核心</span>' : '') + '</div></div>';
    });
    if (alerts.length > 15) body += '<div style="font-size:10.5px;color:var(--text3);text-align:center;padding:4px 0">… 其余 ' + (alerts.length - 15) + ' 条见智能预警中心</div>';
    body += '</div>';
    this._drawerShell(title, act, body);
  },

  /* ── 受威胁项目复合抽屉 ── */
  _openProjectDrawer: function (projName, country) {
    if (!projName) return;
    var self = this;
    var th = null;
    (this._threat || []).forEach(function (r) { if (r.proj === projName) th = r; });
    country = country || (th ? th.country : '');
    var alerts = country ? this._countryAlerts(country) : [];
    var projs = country ? this._countryProjects(country) : [];
    var sibs = projs.filter(function (p) { return p.n !== projName; });
    var me = projs.filter(function (p) { return p.n === projName; })[0] || {};
    var hN = this._hours === 168 ? '7 天' : this._hours + ' 小时';
    var heat = th ? th.heat : 0;
    var heatLv = th && th.red ? '高危' : th && th.orange ? '关注' : alerts.length ? '一般' : '安全窗口';
    var heatCol = th && th.red ? 'var(--red)' : th && th.orange ? 'var(--orange)' : alerts.length ? 'var(--yellow)' : 'var(--green)';
    var title = '<div class="wb-dtitle">🏗 ' + this._esc(projName) + '<span style="font-size:11px;font-weight:400;color:var(--text3)">项目威胁复合详情 · 近 ' + hN + '</span></div>' +
      '<div class="wb-dchips">' +
      '<span class="wb-chip">所在国 <b style="color:var(--text)">' + this._esc(country || '—') + '</b></span>' +
      (me.ent || (th && th.ent) ? '<span class="wb-chip">承建 <b>' + this._esc(me.ent || th.ent) + '</b></span>' : '') +
      (me.inv || (th && th.inv) ? '<span class="wb-chip">投资 <b>' + this._esc(String(me.inv || th.inv)) + '亿</b></span>' : '') +
      (me.p || (th && th.pers && th.pers !== '-') ? '<span class="wb-chip">人员 <b>' + this._esc(String(me.p || (th && th.pers))) + '</b></span>' : '') +
      '<span class="wb-chip" style="border-color:' + heatCol + '55;color:' + heatCol + '">威胁热度 <b>' + heat + '</b>（' + heatLv + '）</span>' +
      (th && th.red ? '<span class="wb-chip" style="border-color:var(--red)55;color:var(--red)">红 <b>' + th.red + '</b></span>' : '') +
      (th && th.orange ? '<span class="wb-chip" style="border-color:var(--orange)55;color:var(--orange)">橙 <b>' + th.orange + '</b></span>' : '') +
      (th && th.core ? '<span class="wb-chip" style="border-color:var(--red)55;color:var(--red)">★核心区事件</span>' : '') +
      '</div>';
    var act = '<button class="wb-btn pri" onclick="WORKBENCH._closeDrawer();WORKBENCH._flyToCountry(\'' + this._esc(country).replace(/'/g, '') + '\')">🗺 地图定位所在国</button>' +
      '<button class="wb-btn" onclick="WORKBENCH._gotoThreatroom(\'' + this._esc(projName).replace(/'/g, '') + '\')">🎯 对项目发起专项作战</button>' +
      '<button class="wb-btn" onclick="WORKBENCH._copyProjNote(\'' + this._esc(projName).replace(/'/g, '') + '\',\'' + this._esc(country).replace(/'/g, '') + '\')">📋 复制防护要点</button>';
    var body = '';
    if (th && th.topTitle) {
      body += '<div class="wb-dsec"><div class="wb-dst">所在国头号威胁</div>' +
        '<div style="font-size:12.5px;color:var(--text2);line-height:1.7;border-left:3px solid ' + heatCol + ';padding-left:10px">' + this._esc(th.topTitle) + '</div></div>';
    }
    body += '<div class="wb-dsec"><div class="wb-dst">所在国预警时间线（' + hN + '）</div>' + this._timelineHTML(alerts) + '</div>';
    if (sibs.length) {
      body += '<div class="wb-dsec"><div class="wb-dst">同国其他建档项目（' + sibs.length + '）</div>';
      sibs.forEach(function (p) {
        body += '<div class="wb-pitem" onclick="WORKBENCH._openProjectDrawer(\'' + self._esc(p.n).replace(/'/g, '') + '\',\'' + self._esc(country).replace(/'/g, '') + '\')">' +
          '<span style="width:9px;height:9px;background:var(--cyan);transform:rotate(45deg);flex-shrink:0"></span>' +
          '<span style="font-weight:700;color:var(--text)">' + self._esc(p.n) + '</span>' +
          '<span style="color:var(--text3);font-size:11px">' + self._esc(p.ent) + (p.inv ? ' · ' + p.inv + '亿' : '') + '</span>' +
          '<span style="margin-left:auto;font-size:10.5px;color:var(--cyan)">切换 →</span></div>';
      });
      body += '</div>';
    }
    body += '<div class="wb-dsec"><div class="wb-dst">所在国关联预警（' + alerts.length + ' 条 · 点击查看全局预警详情）</div>';
    if (!alerts.length) body += '<div style="font-size:11.5px;color:var(--text3);padding:6px 0">时间窗内所在国无预警（安全窗口）</div>';
    alerts.slice(0, 15).forEach(function (a) {
      var lc = self._lvColor(a.level);
      body += '<div class="wb-aitem" style="border-left-color:' + lc + '" onclick="WORKBENCH._openAlert(\'' + String(a.id || '').replace(/'/g, '') + '\')">' +
        '<div class="t">' + self._esc(a.title_zh || a.title || '（无标题）') + '</div>' +
  ...
  /* 抽屉动作：跳专项作战室并自动发起该国/项目专项 */
  _gotoThreatroom: function (q) {
    this._closeDrawer();
    if (typeof navigateTo === 'function') navigateTo('threatroom');
    setTimeout(function () {
      var inp = document.getElementById('tr-q');
      if (inp && q) inp.value = q;
      if (typeof THREATROOM !== 'undefined' && q) { try { THREATROOM.run(q); } catch (e) {} }
    }, 350);
  },
  /* 抽屉动作：打开全局预警详情（复用 app.js showAlertDetail） */
  _openAlert: function (id) {
    this._closeDrawer();
    if (typeof showAlertDetail === 'function') showAlertDetail(id);
  },
  /* 抽屉动作：复制盯防要点（国家） */
  _copyWatchNote: function (country) {
    var alerts = this._countryAlerts(country);
    var w = null;
    (this._watch || []).forEach(function (r) { if (r.country === country) w = r; });
    var L = ['【重点盯防要点】' + country + '（' + new Date().toLocaleString('sv-SE').slice(0, 16) + '）'];
    if (w) L.push('盯防分值 ' + w.score + '；预警 ' + w.n + ' 条（红' + w.red + '/橙' + w.orange + '/黄' + w.yellow + '），涉华 ' + w.cn + ' 条，核心 ' + w.core + ' 条，关联建档项目 ' + w.proj + ' 个' + (w.anom ? '；速率超基线 ' + w.anom.ratio + '×' : ''));
    var projs = this._countryProjects(country);
    if (projs.length) L.push('建档项目：' + projs.map(function (p) { return p.n; }).join('、'));
    L.push('—— 代表性预警 ——');
    alerts.slice(0, 6).forEach(function (a, i) { L.push((i + 1) + '. [' + (a.level || '-') + '] ' + (a.title_zh || a.title || '')); });
    this._copyText(L.join('\n'));
  },
  /* 抽屉动作：复制防护要点（项目） */
  _copyProjNote: function (projName, country) {
    var th = null;
    (this._threat || []).forEach(function (r) { if (r.proj === projName) th = r; });
    var alerts = this._countryAlerts(country);
    var L = ['【项目防护要点】' + projName + '（' + country + '，' + new Date().toLocaleString('sv-SE').slice(0, 16) + '）'];
    if (th) L.push('威胁热度 ' + th.heat + '；所在国预警 ' + th.n + ' 条（红' + th.red + '/橙' + th.orange + '）' + (th.core ? '；含核心区事件' : ''));
    if (th && th.topTitle) L.push('头号威胁：' + th.topTitle);
    L.push('—— 所在国代表性预警 ——');
    alerts.slice(0, 6).forEach(function (a, i) { L.push((i + 1) + '. [' + (a.level || '-') + '] ' + (a.title_zh || a.title || '')); });
    L.push('建议：核实现场人员与营地安防状态，视情升级安保等级并通报使领馆。');
    this._copyText(L.join('\n'));
  },
  _copyText: function (txt) {
    var done = function () { if (typeof showToast === 'function') showToast('要点已复制到剪贴板'); };
  ...
  _bind: function () {
    ...
    /* ── 公文输出（#569《情报会商纪要》） ── */
    var gd = document.getElementById('wb-govdoc');
    if (gd) gd.onclick = function () {
      if (typeof GOVDOC !== 'undefined') GOVDOC.open('workbench');
    };
    /* ── 满宽地图 ── */
...
(function () {
  ...
  st.textContent =
    ...
    /* ── 复合详情抽屉（2026-09-05 用户指令：盯防/项目面板可点开的复合人机交互） ── */
    '.wb-drawer-mask{position:fixed;inset:0;background:rgba(4,8,14,.55);z-index:9500}' +
    '.wb-drawer{position:fixed;top:0;right:0;bottom:0;width:620px;max-width:94vw;background:var(--bg);border-left:1px solid var(--border2);z-index:9501;overflow-y:auto;box-shadow:-18px 0 60px rgba(0,0,0,.55);animation:wbDrIn .22s ease-out}' +
    '@keyframes wbDrIn{from{transform:translateX(40px);opacity:.4}to{transform:none;opacity:1}}' +
    '.wb-dh{position:sticky;top:0;background:var(--bg);border-bottom:1px solid var(--border);padding:14px 18px;z-index:2}' +
    '.wb-dtitle{font-size:16px;font-weight:800;color:var(--text);display:flex;align-items:center;gap:9px}' +
    '.wb-dchips{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}' +
    '.wb-chip{font-size:10.5px;padding:2px 9px;border-radius:3px;border:1px solid var(--border2);color:var(--text2)}' +
    '.wb-chip b{font-weight:700}' +
    '.wb-dact{display:flex;gap:8px;flex-wrap:wrap;padding:11px 18px;border-bottom:1px solid var(--border);background:var(--bg2)}' +
    '.wb-btn{font-size:11.5px;padding:5px 13px;border-radius:6px;border:1px solid var(--border2);background:var(--panel2);color:var(--text2);cursor:pointer}' +
    '.wb-btn:hover{border-color:var(--cyan);color:var(--cyan)}' +
    '.wb-btn.pri{border-color:var(--cyan);color:var(--cyan);background:rgba(0,212,255,.08)}' +
    '.wb-dsec{padding:13px 18px;border-bottom:1px solid var(--border)}' +
    '.wb-dst{font-size:11px;letter-spacing:2px;color:var(--cyan);font-weight:700;margin-bottom:8px}' +
    '.wb-tl{display:flex;align-items:flex-end;gap:2px;height:52px;padding:2px 0}' +
    '.wb-tlb{flex:1;min-width:3px;border-radius:1.5px 1.5px 0 0;background:linear-gradient(180deg,var(--cyan),rgba(0,102,255,.35));cursor:pointer;transition:filter .15s}' +
    '.wb-tlb:hover{filter:brightness(1.6)}' +
    '.wb-aitem{border:1px solid var(--border2);border-left:3px solid var(--text3);border-radius:6px;padding:8px 11px;margin-bottom:7px;cursor:pointer;background:var(--bg2)}' +
    '.wb-aitem:hover{border-color:var(--cyan);background:rgba(0,212,255,.05)}' +
    '.wb-aitem .t{font-size:12.5px;color:var(--text);line-height:1.55}' +
    '.wb-aitem .m{display:flex;gap:8px;align-items:center;margin-top:4px;font-size:10.5px;color:var(--text3);flex-wrap:wrap}' +
    '.wb-pitem{display:flex;gap:9px;align-items:center;padding:8px 10px;border:1px solid var(--border2);border-radius:6px;margin-bottom:6px;font-size:12px;cursor:pointer;background:var(--bg2)}' +
    '.wb-pitem:hover{border-color:var(--cyan)}';
