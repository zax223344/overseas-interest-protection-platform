...
var COLLECTED_DB={
  ...
  init(){
    ...
    // 清洗采集库历史脏数据（网关部署前遗留的国内噪声）——须在 _loadFromAPI 前执行，
    // 避免 API 覆盖把我们刚清干净的本地数据再拉回旧脏数据（API 自身已是网关后数据，不会污染）。
    try{
      if(typeof GATE!=='undefined'&&GATE.chinaOverseasGate){
        var cp=this.purgeNonOverseas();
        if(cp&&cp.total>0) console.log('[COLLECTED_DB] 启动时清理采集库 '+cp.total+' 条非海外利益数据');
      }
    }catch(e){ console.warn('[COLLECTED_DB] 清洗失败:', e); }

    // 异步从后端 API 加载数据
    this._loadFromAPI();
  },

  /* 从 API 异步加载所有类别数据（在线时覆盖本地） */
  _loadFromAPI(){
    var self=this;
    if(typeof APIClient==='undefined'||!APIClient.isOnline())return;
    this.CATEGORIES.forEach(function(cat){
      APIClient.getCollected(cat).then(function(data){
        if(data&&data.items&&Array.isArray(data.items)){
          try{localStorage.setItem(self.PREFIX+cat,JSON.stringify(data.items));}catch(e){}
        }
      }).catch(function(){});
    });
    // 同时加载其他模块数据
    if(typeof ENTERPRISE_DB!=='undefined'){
      APIClient.getEnterpriseProjects().then(function(data){
        if(data&&data.projects&&Array.isArray(data.projects)){
          try{localStorage.setItem(ENTERPRISE_DB.KEY,JSON.stringify(data.projects));}catch(e){}
        }
      }).catch(function(){});
    }
    if(typeof RISK_FUSION!=='undefined'){
      APIClient.getRiskFusion().then(function(data){
        if(data&&data.fusions&&Array.isArray(data.fusions)){
          try{localStorage.setItem(RISK_FUSION.KEY,JSON.stringify(data.fusions));}catch(e){}
        }
      }).catch(function(){});
    }
    if(typeof THREAT_ORGS_DB!=='undefined'){
      APIClient.getThreatOrgs().then(function(data){
        if(data&&data.orgs&&Array.isArray(data.orgs)){
          try{localStorage.setItem(THREAT_ORGS_DB.KEY,JSON.stringify(data.orgs));}catch(e){}
        }
      }).catch(function(){});
    }
  },
  ...
  _w(k,v){try{localStorage.setItem(this.PREFIX+k,JSON.stringify(v));}catch(e){}this._syncAPI(k,v);},
  _syncAPI(k,v){if(typeof APIClient!=='undefined'&&APIClient.isOnline()&&APIClient.getToken()){APIClient.saveCollected(k,v).catch(function(){});}},

  /* 体裁闸门（采集库唯一咽喉）：商业榜单/经济统计、评论/学术论述不是海外利益安全数据。
   * 设在 add/addBatch 而非各调用点，是因为 datasources.js 有 4 条采集路径（_refreshReal、
   * realBurst、crawlBurst 等）都写这里，逐个打补丁必然漏。返回 true 表示应丢弃。 */
  _genreBlocked(item){
  ...
  add(category,item){
    ...
    item.audit_status='approved'; /* 自动审核：采集即通过 */
    item.audit_time=new Date().toISOString();
    var a=this._r(category);
    a.unshift(item);
    this._w(category,a);
    /* 实时流转：直接入 DBCenter + 立即分发全系统（不再等15s自循环） */
    try{
      if(typeof DBCenter!=='undefined'){
        var _t={}; for(var _k in item) _t[_k]=item[_k];
        _t._distributed=true; delete _t.id;
        DBCenter.add(category,_t);
      }
    }catch(e){}
    try{ if(typeof _ingestApproved==='function') _ingestApproved(item,category); }catch(e){}
    try{ if(typeof DATACENTER!=='undefined'&&DATACENTER._refreshAllViews) DATACENTER._refreshAllViews(); }catch(e){}
    return item.id;
  },
  ...
  addBatch(category,items){
    ...
    this._w(category,a);
    /* 实时流转：整批直接入 DBCenter + 逐条分发全系统 */
    try{
      if(typeof DBCenter!=='undefined'){
        var _batch=list.map(function(item){
          var _t={}; for(var _k in item) _t[_k]=item[_k];
          _t._distributed=true; delete _t.id;
          return _t;
        });
        DBCenter.addBatch(category,_batch);
      }
    }catch(e){}
    try{
  ...
  /* 清洗采集库历史脏数据：与 DBCenter.purgeNonOverseas 同源，剔除纯国内噪声。
   * 采集库是 DBCenter 的上游（_approvedSyncScan 每 15s 镜像 pending 进 DBCenter），
   * 不清采集库则脏数据会持续回流 DBCenter。 */
  purgeNonOverseas(){
    ...
    this.CATEGORIES.forEach(function(cat){
      ...
      for(var i=0;i<data.length;i++){
        ...
        /* 体裁闸门：商业榜单/经济统计、评论/学术论述一票否决（存量逐条重判） */
    ...
    if(total>0) console.log('[COLLECTED_DB.purgeNonOverseas] 清理 '+total+' 条：', byCat);
  ...
  deleteBatch(category,ids){
    if(!ids||!ids.length)return 0;
    var data=this._r(category);
    var before=data.length;
    var filtered=data.filter(function(d){return ids.indexOf(d.id)<0;});
    this._w(category,filtered);
  ...
  /* 标记某条数据"已分发"（转入主库后由 _approvedSyncScan 跳过，防重复弹预警） */
  markDistributed(category,id){
    var data=this._r(category);
    var item=data.find(function(d){return d.id===id;});
    if(!item)return false;
    item._distributed=true;
    this._w(category,data);
    return true;
  },
  ...
  transferToDBCenter(category,id){
    ...
    transferItem._distributed=true; /* 审核时已分发预警，主库中不再重复分发 */
  ...
  // ===== 模拟数据工具（已停用：实战模式不再注入任何模拟数据） =====
  SIM_VERSION:'1.0',
  SIM_KEY:'orps_sim_data_version',

  seedSimulatedData(){
    // 实战模式：不再生成任何模拟采集数据，仅使用真实抓取数据
    return 0;
  },
...
var ENTERPRISE_DB={
  ...
  VERSION:'1.1', /* 1.1: 种子项目补齐稳定id（修复风险融合按项目聚合失真） */

  init(){
    var ver=localStorage.getItem(this.VER_KEY);
    if(ver!==this.VERSION){
      var seed=this._seedData();
      /* 版本升级时保留用户手工添加的项目（带 created_at 标记） */
      var userAdded=[];
      try{userAdded=(JSON.parse(localStorage.getItem(this.KEY)||'[]')||[]).filter(function(p){return p&&p.created_at;});}catch(e){}
      localStorage.setItem(this.KEY,JSON.stringify(seed.concat(userAdded)));
      localStorage.setItem(this.VER_KEY,this.VERSION);
      console.log('[ENTERPRISE_DB] Initialized v'+this.VERSION+' with '+seed.length+' projects'+(userAdded.length?(' + 保留用户项目 '+userAdded.length+' 个'):''));
    }
  },

  _r(){try{return JSON.parse(localStorage.getItem(this.KEY)||'[]');}catch(e){return[];}},
  _w(v){try{localStorage.setItem(this.KEY,JSON.stringify(v));}catch(e){}this._syncAPI(v);},
  ...
  // 种子数据: 一带一路重大项目 + 中资企业海外核心项目（真实公开项目，非模拟）
  _seedData(){
    var list=this._seedRaw();
    /* 补齐稳定 id（1001 起递增），保证融合结果能按项目正确聚合与打开详情 */
    list.forEach(function(p,i){if(!p.id)p.id=1001+i;});
    return list;
  },
  _seedRaw(){
    return [
...
var RISK_FUSION={
  ...
  init(){
    ...
    // 实战模式：不自动注入任何模拟融合结果，仅基于真实采集数据执行融合
  },
  ...
  // 事件严重度权重（含中文严重度：真实采集数据为 高/中/低）
  SEVERITY_WEIGHT:{
    'critical':100,'high':80,'red':80,'orange':50,'medium':40,'yellow':30,'low':15,
    '紧急':100,'高':80,'中':40,'低':15
  },

  // 国家别名归一化（真实采集数据常用简称/英文名，项目库用全称）
  COUNTRY_ALIAS:{
    '印尼':'印度尼西亚','China':'中国','Russia':'俄罗斯','俄国':'俄罗斯',
    'Pakistan':'巴基斯坦','Myanmar':'缅甸','Burma':'缅甸','France':'法国',
    'Indonesia':'印度尼西亚','Malaysia':'马来西亚','Thailand':'泰国','Laos':'老挝',
    'Vietnam':'越南','Cambodia':'柬埔寨','Kenya':'肯尼亚','Ethiopia':'埃塞俄比亚',
    'Nigeria':'尼日利亚','Greece':'希腊','Turkey':'土耳其','土耳其共和国':'土耳其',
    'Iran':'伊朗','Saudi Arabia':'沙特阿拉伯','沙特':'沙特阿拉伯','UAE':'阿联酋',
    'Kazakhstan':'哈萨克斯坦','Brazil':'巴西','Peru':'秘鲁','Argentina':'阿根廷',
    'Chile':'智利','Australia':'澳大利亚','Sri Lanka':'斯里兰卡','Bangladesh':'孟加拉国',
    '刚果金':'刚果(金)','刚果民主共和国':'刚果(金)','DRC':'刚果(金)','Guinea':'几内亚',
    'Zambia':'赞比亚','Angola':'安哥拉','Hungary':'匈牙利','Serbia':'塞尔维亚',
    'Djibouti':'吉布提','Turkmenistan':'土库曼斯坦','Uzbekistan':'乌兹别克斯坦'
  },
  _normCountry(c){
    c=(c||'').trim();
    if(!c)return '';
    return this.COUNTRY_ALIAS[c]||c;
  },
  ...
  calculateMatchScore(event,project){
    ...
    // 1. 国家匹配 (核心) — 别名归一化 + 多国项目(用/分隔)拆分匹配
    var eventCountry=this._normCountry(event.country);
    var projectCountries=(project.country||'').split(/[\/、,，]/).map(function(c){return RISK_FUSION._normCountry(c);}).filter(function(c){return c;});
    if(eventCountry&&projectCountries.length){
      var exact=projectCountries.some(function(pc){return pc===eventCountry;});
      var partial=!exact&&projectCountries.some(function(pc){return pc.indexOf(eventCountry)>=0||eventCountry.indexOf(pc)>=0;});
      if(exact){
        score+=50;
        reasons.push('同国家: '+eventCountry);
      }else if(partial){
        score+=45;
        reasons.push('国家关联: '+eventCountry+' ↔ '+project.country);
      }else{
        // 检查是否同一地区(如事件国家在项目国家附近)
        var region=this._getRegion(eventCountry);
        var sameRegion=projectCountries.some(function(pc){var r=RISK_FUSION._getRegion(pc);return region&&r===region;});
        if(sameRegion){
          score+=15;
  ...
  fuse(){
    ...
    var categories=['terror_events','security_events','military_conflicts','political_events',
      'natural_disasters','public_health','sanctions_data','social_unrest',
      'infrastructure','geopolitical_intel','osint_intel'];
    // 融合=研判工具，覆盖数据中心全部真实条目（仅排除被驳回的）；
    // 审核闸门只控制"是否分发预警中心"，不影响风险融合研判
    categories.forEach(function(cat){
      var data=DBCenter.getAll(cat);
      data.forEach(function(d){
        if(d.audit_status==='rejected')return;
        if(d._sim===true||d.is_simulated===true)return; // 铁律：绝不融合模拟数据
        d._collection=cat;
        events.push(d);
      });
    });
      ...
      projects.forEach(function(project){
        ...
        if(result.score>=30){ // 阈值: 分数≥30才算有关联
          matches.push({
            ...
            event_date:event.event_date||event.time||event.date||'',
            event_type:event._collection||'',
            event_audit:event.audit_status||'approved',
    ...
    // 按匹配分数排序，并限制上限防 localStorage 膨胀
  ...
  // ===== 融合结果工具（已停用模拟注入：实战模式仅基于真实采集执行融合） =====
  SIM_VERSION:'1.0',
  SIM_FUSION_KEY:'orps_sim_fusion_version',

  seedSimulatedFusion(){
    // 实战模式：不再生成任何模拟融合结果，仅基于真实采集数据与真实项目库执行融合
    return 0;
  },
...
var THREAT_ORGS_DB={
  ...
  init(){
    ...
    // 实战模式：不自动注入任何模拟威胁组织数据，仅展示真实来源数据
  },
...
var SCRAPER={
  ...
  async _fetch(url,useProxy,timeout){
    ...
    // 优先走同源后端 /api/scrape（无 CORS，真实抓取），仅放白名单主机
    try{
      var base=(typeof APIClient!=='undefined'&&APIClient._baseUrl)?APIClient._baseUrl:(typeof window!=='undefined'&&window.location?window.location.origin:'');
      if(base){
        var r=await fetch(base+'/api/scrape?url='+encodeURIComponent(url),{signal:AbortSignal.timeout(timeout)});
        if(r.ok){var j=await r.json(); if(j&&j.ok&&j.text) return j.text;}
      }
    }catch(e){}
    // 回退：直连 / 公共代理（保持旧行为）
    if(!useProxy){
