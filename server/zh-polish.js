...
const ABBR = {
  ...
  'CPEC': '中巴经济走廊',
  'CCECC': '中国土木工程集团',
  /* 2026-09-04 #581 扩充（用户实测「WTI公司」类病句根因：缩写被机翻当普通词拆译） */
  'TTP': '巴基斯坦塔利班',
  'DSS': '尼日利亚国家安全局',
  'ICE': '美国移民与海关执法局',
  'FG': '尼日利亚联邦政府',          /* Federal Government，尼日利亚新闻高频 */
  'AGF': '联邦总检察长',             /* Attorney-General of the Federation，尼日利亚 */
  'INSS': '国家社会保障局',
  'OPEC': '欧佩克',
  'IEA': '国际能源署',
  'EIA': '美国能源信息署',
...
const TICKER_LIKE = { WTI: 'WTI原油', Brent: '布伦特原油', API: 'API' };

/* 词组级硬译错（2026-09-04 #581：用户实测"石油包装"=Oil Wrap 误译） */
const TERM_FIX = [
...
/* ============ 尾置因果从句重排（机翻高频病句，2026-09-04 #577） ============
 * "…again as birthright debate intensifies" 直译为 "美国立法者再次攻击中国，因为出生公民权辩论加剧"——
 * 汉语规范语序是因前果后。规则：主句，(因为|由于)原因 → 因原因，主句。
 * 守卫：①主句 6-40 字、原因 4-24 字且均不含句内标点（整句仅一个逗号，防长句误伤）；
 *      ②主句不以"因/由于/因为"开头（幂等：改写结果不会再命中）；
 *      ③原因不含"因为/由于/随着"（防嵌套从句错排）；④主句与原因均须含汉字（纯英文不动）。 */
function fixCausal(t) {
  const m = String(t || '').match(/^([^，。！？；、]{6,40})，(因为|由于)([^，。！？；、]{4,24})$/);
  if (!m) return t;
  const main = m[1], cause = m[3];
  if (/^(因|由于|因为)/.test(main)) return t;
  if (/因为|由于|随着/.test(cause)) return t;
  if (!/[\u4e00-\u9fa5]/.test(main) || !/[\u4e00-\u9fa5]/.test(cause)) return t;
  return '因' + cause + '，' + main;
}
...
function polish(text) {
  ...
  /* 词组级硬译错（#581：石油包装→石油综述等） */
  for (const [re, to] of TERM_FIX) t = t.replace(re, to);
  /* 军事术语纠正（上下文守卫，见 MILITARY_FIX 注释） */
  t = fixMilitary(t);
  /* 尾置因果从句重排（见 fixCausal 注释） */
  t = fixCausal(t);
  return t.trim();
}

/* ============ #581 标题病句闸（2026-09-04 用户指令：「石油包装：随着海湾战争的加深，WTI公司」
 * 类病句不得再出现）============
 * 三类机翻硬伤修复：
 *   ①「X公司」病句——机翻把 firms（坚挺，动词）/期货缩写当"公司"："WTI公司"→"WTI原油"；
 *     其他缩写误加"公司"的一律去"公司"保留缩写（"XYZ公司"→"XYZ"）；
 *   ②纯数字残尾——"- 36"（媒体栏目页码）剥除；
 *   ③「缩写+团队/集团/当局」同类误构一并去尾。规则幂等，可安全重跑。 */
function fixBrokenTitle(t) {
  let s = String(t || '');
  if (!s) return s;
  /* 「AI公司」「IT公司」等通用技术缩写是合法构词，绝不触碰 */
  const KEEP = /\b(?:AI|IT|3D|5G|VR|AR)公司/g;
  const keep = [];
  s = s.replace(KEEP, m => { keep.push(m); return ' ' + (keep.length - 1) + ' '; });
  s = s.replace(/(?<![A-Za-z0-9])([A-Z]{2,6})公司/g, (m, ab) => TICKER_LIKE[ab] || ab);
  s = s.replace(/(?<![A-Za-z0-9])([A-Z]{2,6})(?:团队|集团|当局)(?=$|[，。；、])/g, (m, ab) => TICKER_LIKE[ab] || ab);
  s = s.replace(/ (\d+) /g, (m, i) => keep[+i]);
  /* 尼日利亚语境高频词组：LG主席 → 地方政府主席（Local Government chairman） */
  s = s.replace(/(?<![A-Za-z0-9])LG主席/g, '地方政府主席');
  /* 媒体自称尾巴：「|独家」「-原创」「|编译」等（2026-09-04 用户实测「…升级暴力|独家」入库） */
  s = s.replace(/[\s\-–—|｜]\s*(?:独家|原创|编译|前瞻|深度|观察|速递|特稿|专稿|首发)\s*$/, '');
  /* 巴基斯坦外交部缩写：句尾「：FO」「—FO」→ 巴外交部（Foreign Office，巴媒高频） */
  s = s.replace(/([：:，,\-–—])\s*FO\s*$/, '$1巴外交部');
