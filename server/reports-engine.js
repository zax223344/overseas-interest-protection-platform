...
const RS = require('./report-standard'); /* #625 统一报告标准：手册规范+GB/T 9704 版式+完稿质检单一来源 */
 ...
 * ============================================================ */
const _ANOM_VIOLENT = new Set(['terror_events', 'security_events', 'military_conflicts', 'crime_events', 'mass_violence']);
...
const _ANOM_CAT_LABELS = (function () {
  const m = {
    ...
    /* 分类体系 v2.0（#627，2026-09-05）：18 子类中文标签，经 CAT_STD 覆盖（新 key 缺映射同样出英文残留） */
    mass_violence: '群体性暴力', crime_events: '社会治安事件', regime_change: '政权变动', election_events: '选举事件',
    policy_shift: '政策法规突变', financial_market: '金融市场风险', business_climate: '营商环境恶化',
    industrial_accident: '生产安全事故', environmental_event: '环境生态事件'
  };
  try {
    const STD = require('./category-standard');
    (STD.SUBCATS || []).forEach(s => { m[s.key] = s.label; });
  } catch (e) {}
  return m;
})();
...
/* #625 统一报告标准：persona+公文语体+手册规范全部由 report-standard.js 供给（禁内联复制） */
const SYSTEM_PROMPT = RS.SYSTEM_PROMPT;
/* 客观数据 → user prompt */
...
async function runLlm(def, periodKey, win, data) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    if (r && r.text) {
      RS.qcLog('reports:' + def.id + ':' + periodKey, r.text); /* #625 完稿质检：Markdown残留/模糊词/口号式说教告警 */
      return { ok: true, text: stripMarkdown(r.text), model: pv.model };
    }
...
function renderHtml(def, periodKey, data, llmText, llmOk) {
  ...
  return '<style>'
    ...
    + '.rp-row{transition:background .15s;}'
    + '.rp-row:hover td{background:rgba(34,211,238,.07);}'
...
async function generateReport(typeId, periodKey, opts) {
  ...
  const o = opts || {};
  /* 2026-09-05 regen 周期错配 root fix：「重新生成」按既有期次键发起但不带 freq（前端契约
   * reports-center.js #528），而期次键本身携带频率信息（2026-09-03=日 / 2026-W36=周 /
   * 2026-Q3=季 / 2026-S1=半年 / 2026=年 / 2026-09=月）。不推断就落类型默认窗口：
   * 日报期次进 weekWindow 直接抛"周期格式错误（应为 YYYY-Wnn）"→ 跨周期 regen 全灭
   * （生产日志实证）。显式 freq 优先；缺省时从期次键推断，与下方标题命名 effFreq 同源。 */
  const freq = (o.freq && FREQ_ALL.indexOf(o.freq) >= 0) ? o.freq
    : (FREQ_ALL.indexOf(freqOfPeriodKey(periodKey)) >= 0 ? freqOfPeriodKey(periodKey) : '');
