...
function _isDomesticChina(text) {
  ...
  if (/中资|中企|华人|华侨|华裔|中方员工|中方人员|中国公民|中国劳工|中国工人|留学生|使馆|领事馆|驻外|海外|境外|国外|一带一路|瓜达尔|中资营地|南中国海|波斯湾|几内亚湾|Chinese national|Chinese worker|Chinese citizen|Chinese compan|overseas Chinese|Chinese employees/i.test(txt)) return false;
...
module.exports = {
  ...
  scrapeTerrorAttacks, _extractLocation
};
...
/* ═══════════ #564 P2-2：地点提取器（terror/kidnap location 100% 空 → 采集侧实时提取） ═══════════
 * 零模拟铁律：只提取标题中真实出现的地名；提取不到保持空串，绝不猜测回填。
 * 两级策略：① 高危冲突区地点白名单（词组精确命中，覆盖 90% 场景）
 *          ② in/near/at <Place> 模式兜底（专名首字母大写 + 黑名单剔除星期/月份/机构词） */
const _LOC_WHITELIST = [
  /* 阿富汗-巴基斯坦 */
  'Kabul','Kandahar','Helmand','Kandahari','Quetta','Karachi','Peshawar','Lahore','Balochistan','Waziristan','Khyber Pakhtunkhwa','Khyber','FATA','Gilgit',
  /* 索马里-非洲之角 */
  'Mogadishu','Somaliland','Puntland','Jubaland','Hargeisa','Kismayo',
  /* 尼日利亚-乍得湖 */
  'Kano','Maiduguri','Borno','Yobe','Adamawa','Zamfara','Kaduna','Katsina','Plateau','Benue','Niger Delta','Lake Chad','Abuja','Lagos','Sokoto','Jigawa',
  /* 萨赫勒 */
  'Bamako','Gao','Timbuktu','Mopti','Menaka','Segou','Niamey','Diffa','Tillaberi','Ouagadougou','Bobo-Dioulasso','Djibo','Sahel',
  /* 刚果-莫桑比克-中非 */
  'Goma','Bukavu','Beni','North Kivu','Ituri','Katanga','Kasai','Ecuador province','Cabo Delgado','Mocimboa','Palma','Niassa','Bangui','Batangafo',
  /* 伊拉克-叙利亚-也门 */
  'Baghdad','Mosul','Kirkuk','Anbar','Fallujah','Damascus','Aleppo','Idlib','Raqqa','Deir ez-Zor','Sanaa','Aden','Hodeidah','Taiz','Marib',
  /* 苏丹-埃及-利比亚-西奈 */
  'Khartoum','Omdurman','Darfur','El Fasher','El Geneina','West Darfur','South Kordofan','Sinai','North Sinai','Benghazi','Tripoli','Sabha',
  /* 埃塞-缅甸-其他 */
  'Amhara','Tigray','Oromia','Afder','Rakhine','Shan','Kachin','Kayin'
];
const _LOC_BLACKLIST_RE = /^(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday|January|February|March|April|May|June|July|August|September|October|November|December|Army|Police|Government|Minister|President|Defence|Defense|Islamic|Muslim|Christian|Authorities|Official|Report|Update|Breaking|Military|Terrorists|Gunmen|Rebels|Insurgents|Protesters|Villagers|Residents|Farmers|Wounded|Killed|China|Chinese|Beijing|Shanghai|Africa|Asia|Europe|America|World|City|Town|Village|Insecurity|Security|Crime|Banditry|Insurgency|Terrorism|Kidnapping|Attack|Attacks|Clash|Clashes|Violence|Crisis|Conflict|War|Fresh|Latest|Two|Three|Four|Five|Six|Seven|Eight|Nine|Ten|Several|Many|New|Ethiopia|Nigeria|Mali|Niger|Pakistan|Afghanistan|Somalia|Kenya|Sudan|Libya|Egypt|Iraq|Syria|Yemen|Congo|Myanmar|Burma|India|Mozambique|Cameroon|Chad|Bangladesh|Uganda|Tanzania|Zambia|Zimbabwe|Ghana|Senegal|Guinea|Benin|Togo|Burundi|Rwanda|Africa)$/i;
function _extractLocation(rawTitle) {
  var t = String(rawTitle || '')
    .replace(/&#0?39;|&apos;/g, "'").replace(/&#8217;/g, "'")
    .replace(/&#8216;/g, "'").replace(/&#8220;|&#8221;/g, '"')
    .replace(/&amp;/g, '&').replace(/&quot;/g, '"');
  if (!t) return '';
  /* ① 白名单词组优先（长词组优先排序避免子串遮蔽） */
  var wl = _LOC_WHITELIST.slice().sort(function (a, b) { return b.length - a.length; });
  for (var i = 0; i < wl.length; i++) {
    var w = wl[i];
    if (new RegExp('\\b' + w.replace(/ /g, '\\s+') + '\\b').test(t)) return w;
  }
  /* ② in/near/at/outside (the) <Place> 模式：1-3 个首字母大写词，方位修饰词前置可选 */
  var m = t.match(/\b(?:in|near|at|outside)\s+(?:the\s+)?(?:northern\s+|southern\s+|eastern\s+|western\s+|north\s+|south\s+|east\s+|west\s+|central\s+|northwestern\s+|southwestern\s+)?([A-Z][A-Za-z]{2,}(?:\s+(?:de|del|el|al|of)?\s*[A-Z][A-Za-z]{2,}){0,2})/);
  if (m && m[1]) {
    var place = m[1].trim();
    var first = place.split(/\s+/)[0];
    if (!_LOC_BLACKLIST_RE.test(first) && place.length >= 4 && place.length <= 40) return place;
  }
  /* ③ 前缀 "Place:" 标题（媒体常见 "MAIDUGURI: Gunmen ..."） */
  var m2 = t.match(/^([A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z]+){0,2}):\s/);
  if (m2 && m2[1] && !_LOC_BLACKLIST_RE.test(m2[1].split(/\s+/)[0]) && m2[1].length >= 4) return m2[1].trim();
  return '';
}
      ...
      for (const arts of gdeltRes) {
        for (const a of arts || []) {
          ...
          tagged.push({
            title: a.title, content: '', url: a.url,
            country_cn: c.cn, country_iso: c.iso,
            location: _extractLocation(a.title),   /* #564：英文标题提取地点（提取不到为空，不猜测） */
      ...
      const rssResults = await _batchRun(rssSources, async s => {
        ...
        for (const it of parsed) {
          ...
          tagged2.push({
            title: it.title, content: it.description || '', url: it.link || s.url,
            country_cn: c.cn, country_iso: c.iso,
            location: _extractLocation(it.title),  /* #564：RSS 腿同样提取 */
        ...
        for (const arts of kidnapRes) {
          for (const a of arts || []) {
            ...
            tagged.push({
              title: a.title, content: '', url: a.url,
              country_cn: c.cn, country_iso: c.iso,
              location: _extractLocation(a.title),   /* #564：绑架案腿 */
