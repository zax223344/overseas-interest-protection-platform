...
var THREATROOM = {
  ...
  DT_CN: (function () {
    ...
    /* 分类体系 v2.0（#627）：18 子类标签经 CATV2 覆盖（threatroom.js 后于 app.js 加载） */
    if (typeof CATV2 !== 'undefined' && CATV2.labelOf) { for (var k in CATV2.labelOf) m[k] = CATV2.labelOf[k]; }
    return m;
  })(),
  ...
  _shellHTML: function () {
    ...
    s += '<button class="tr-go" id="tr-govdoc" style="background:linear-gradient(135deg,#7c3aed,#a855f7)" title="基于当前实体监测窗口数据，生成公文《威胁通报》">📄 公文输出</button>';
    s += '</div>';
  ...
  _bindShell: function () {
    ...
    /* 公文输出（#569《威胁通报》：需先跑过一轮实体监测） */
    var gd = document.getElementById('tr-govdoc');
    if (gd) gd.onclick = function () {
      if (typeof GOVDOC === 'undefined') return;
      if (!self._entity) { self._toast('请先「启动作战」完成一轮实体监测，再输出《威胁通报》公文'); return; }
      GOVDOC.open('threatroom');
    };
    if (q) {
  ...
  _renderReport: function () {
    ...
    html += '<div class="card-tt"><span class="ic">🇨🇳</span>涉华焦点<span style="margin-left:auto;font-weight:400;font-size:11px;color:var(--text3,#7a8aa3)">' + st.cn + ' 条 · 点击开复合详情</span></div>';
    ...
    /* 关联项目资产 */
    html += '<div class="card" style="flex:1;min-width:320px;padding:12px">';
    html += '<div class="card-tt"><span class="ic">🏗️</span>关联中资项目资产<span style="margin-left:auto;font-weight:400;font-size:11px;color:var(--text3,#7a8aa3)">ENTERPRISES 档案联动 · 点击开项目复合详情</span></div>';
    var projs = this._linkProjects();
    if (projs.length) {
      projs.slice(0, 8).forEach(function (p) {
        var heat = st.byCountry[p.c] || 0;
        html += '<div class="tr-proj" data-wc="' + THREATROOM._esc(p.c) + '" data-proj="' + THREATROOM._esc(p.n) + '">' +
    ...
    Array.prototype.forEach.call(rep.querySelectorAll('.tr-proj[data-wc]'), function (el) {
      el.onclick = function () { self._projDetail(el.getAttribute('data-proj') || ''); };
    });
  ...
  /* ═══════════ 复合详情弹窗（2026-09-05 用户指令二：交互太简单 → 复合人机交互） ═══════════
   * ① 情报要素卡：信源/时间/类别/国别/等级/涉华/核心/全网标；
   * ② 正文全文（实体词高亮）；
   * ③ 规则研判段：类型×等级×国别 → 实战处置建议；
   * ④ 该国时间线（窗口内逐日量）+ 关联情报（同国/同类，点击切换）；
   * ⑤ 动作区：原文 / 推送预警中心 / 复制要点 / 转指挥事件。 */
  _detail: function (i) {
    ...
    var html = '<div class="tr-modal" id="tr-modal" onclick="if(event.target===this)THREATROOM._closeDetail()"><div class="tr-modalbox" style="max-width:820px">';
    /* ① 要素卡 */
    html += '<div style="display:flex;align-items:flex-start;gap:10px">';
    html += '<div style="flex:1;font-size:15.5px;font-weight:700;color:var(--text,#e8eefc);line-height:1.6">' + this._hl(this._title(it), e) + '</div>';
    html += '<button onclick="THREATROOM._closeDetail()" style="background:none;border:1px solid var(--border2,#1e2a40);color:var(--text2,#aab8d0);border-radius:6px;padding:4px 12px;cursor:pointer;flex-shrink:0">✕ 关闭</button></div>';
    html += '<div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:10px;font-size:11px;color:var(--text3,#7a8aa3)">' +
      '<span class="tr-badge" style="border-color:var(--border2,#1e2a40)">' + this._esc(it.source || '—') + '</span>' +
      '<span class="tr-badge" style="border-color:var(--border2,#1e2a40)">' + this._when(it) + '</span>' +
      '<span class="tr-badge" style="border-color:var(--border2,#1e2a40)">' + (this.DT_CN[it.data_type] || it.data_type || '') + '</span>' +
      (it.country ? '<span class="tr-badge" style="border-color:var(--cyan,#00d4ff)55;color:var(--cyan,#00d4ff)">📍' + this._esc(it.country) + '</span>' : '') +
      (it.level ? '<span class="tr-badge" style="color:' + lc + ';border-color:' + lc + '55">' + this._lvCN(it.level) + '级</span>' : '') +
      (it.chinaRelated === true ? '<span class="tr-badge" style="color:var(--orange,#f59e0b);border-color:var(--orange,#f59e0b)55">涉华</span>' : '') +
      (it.is_core ? '<span class="tr-badge" style="color:var(--red,#ef4444);border-color:var(--red,#ef4444)55">★核心</span>' : '') +
      (it._web ? '<span class="tr-badge" style="color:var(--green,#22c55e);border-color:var(--green,#22c55e)55">🌐 本次全网</span>' : '') + '</div>';
    /* ② 正文 */
    var desc = String(it.description || it.content || it.desc || it.summary || '').trim();
    if (desc) {
      desc = desc.replace(/<[^>]+>/g, '').slice(0, 3000);
      html += '<div style="margin-top:12px;font-size:12.5px;line-height:1.9;color:var(--text2,#aab8d0);white-space:pre-wrap;max-height:300px;overflow-y:auto">' + this._hl(desc, e) + '</div>';
    } else html += '<div style="margin-top:12px;font-size:11.5px;color:var(--text3,#7a8aa3)">（正文未入库：检索通道仅返回标题，可点原文链接查看）</div>';
    /* ③ 规则研判段 */
    html += this._assessHTML(it);
    /* ④ 时间线 + 关联情报 */
    if (it.country) html += this._countryTlineHTML(it.country, i);
    html += this._relatedHTML(it, i);
    /* ⑤ 动作区 */
    html += '<div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:14px;border-top:1px solid var(--border2,#1e2a40);padding-top:12px">';
    if (it.url) html += '<a href="' + this._esc(it.url) + '" target="_blank" rel="noopener" style="color:var(--cyan,#00d4ff);font-size:12px">🔗 查看原文 →</a>';
    html += '<span style="flex:1"></span>';
    html += '<button onclick="THREATROOM._copyIntelNote(' + i + ')" style="background:rgba(0,212,255,0.08);border:1px solid var(--border2,#1e2a40);color:var(--text2,#aab8d0);border-radius:6px;padding:5px 13px;cursor:pointer;font-size:12px">📋 复制要点</button>';
    if (typeof COMMAND !== 'undefined') html += '<button onclick="THREATROOM._toCommand(' + i + ')" style="background:rgba(239,68,68,0.10);border:1px solid var(--red,#ef4444);color:var(--red,#ef4444);border-radius:6px;padding:5px 13px;cursor:pointer;font-size:12px;font-weight:700">🎖 转指挥事件</button>';
  ...
  /* 规则研判：类型×等级×国别×涉华 → 处置建议（纯前端确定性推导，零模拟） */
  _assessHTML: function (it) {
    var L = [];
    var dt = it.data_type || '';
    var lv = it.level || 'blue';
    var cn = it.chinaRelated === true;
    var dtCn = this.DT_CN[dt] || '开源情报';
    /* 威胁定性 */
    var sev = lv === 'red' ? '高级别威胁信号' : lv === 'orange' ? '显著风险信号' : lv === 'yellow' ? '一般风险信号' : '背景性信息';
    L.push('本条为「' + dtCn + '」类' + sev + (it.country ? '，事发/涉及 ' + it.country : '') + '。');
    /* 类型化建议 */
    var adv = {
      terror_events: '建议立即核对事发地中方人员/营地位置与安防状态，通报驻外使领馆领保渠道，视情暂停现场作业并升级营地封控。',
      security_events: '建议比对涉华人员/项目暴露面，核实是否有中方公民或中资企业卷入，保持领保热线值守。',
      military_conflicts: '建议评估交战区与中方项目/航路距离，检视撤离预案可用性，关注外溢风险与次生灾害。',
      political_events: '建议研判政权/政策变动对在建合同与人员签证的影响，暂缓重大投资决策，跟踪后续组阁与政策宣示。',
      geopolitical_intel: '建议纳入国别风险档案滚动更新，关注对我战略通道与供应链的传导效应。',
      natural_disasters: '建议立即清点灾区中方人员与项目设施，启动自救互救，预置撤离运力与应急物资。',
      public_health: '建议发布防疫提示，核查中方人员疫苗接种与防疫物资储备，视情实施营地封闭管理。',
      sanctions_data: '建议合规部门立即排查涉事实体与中方业务敞口，评估二级制裁传导风险，准备替代结算与供应方案。',
      social_unrest: '建议暂停事发区域外出与集会活动，加固项目营地安防，跟踪事态规模与当局处置力度。',
      infrastructure: '建议核实受损/涉险基础设施与中方工程承包、矿产权益、运输通道的关联度，评估工期与物流影响。',
      osint_intel: '建议交叉印证信源可靠性，纳入专题监测清单持续跟踪。'
    };
    /* v2.0 新 key 复用同域建议文案 */
    adv.crime_events = adv.security_events; adv.mass_violence = adv.security_events;
    adv.regime_change = adv.political_events; adv.election_events = adv.political_events; adv.policy_shift = adv.political_events;
    adv.financial_market = adv.sanctions_data; adv.business_climate = adv.sanctions_data;
    adv.industrial_accident = adv.infrastructure; adv.environmental_event = adv.natural_disasters;
    adv.cyber_security = adv.public_health;
    if (adv[dt]) L.push(adv[dt]);
    if (cn) L.push('涉华标记：本条与中国公民/企业/项目存在直接关联，优先级上调，建议 2 小时内完成初核。');
    if (it.is_core) L.push('核心区事件：位于中方核心利益区域，建议直报值班领导并启动专项跟踪。');
    return '<div style="margin-top:12px;background:rgba(0,212,255,0.05);border:1px solid rgba(0,212,255,0.18);border-radius:8px;padding:11px 13px">' +
      '<div style="font-size:10.5px;letter-spacing:2px;color:var(--cyan,#00d4ff);font-weight:700;margin-bottom:5px">⚡ 研判处置建议</div>' +
      '<div style="font-size:12px;color:var(--text2,#aab8d0);line-height:1.85">' + L.map(function (x) { return '<div>' + x + '</div>'; }).join('') + '</div></div>';
  },

  /* 该国窗口内逐日时间线（基于本次作战 _items） */
  _countryTlineHTML: function (country, selfIdx) {
    ...
    for (var i = days - 1; i >= 0; i--) {
      ...
      buckets[k] = 0;
      labels.push({ k: k, lab: (d.getMonth() + 1) + '/' + d.getDate() });
    }
    (this._items || []).forEach(function (x) {
      if (!x || x.country !== country) return;
      var t = THREATROOM._tsOf(x); if (!t) return;
      var d = new Date(t);
      var k = d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate();
      if (buckets[k] != null) buckets[k]++;
    });
    var max = Math.max.apply(null, labels.map(function (l) { return buckets[l.k]; }).concat([1]));
    var html = '<div style="margin-top:12px"><div style="font-size:10.5px;letter-spacing:2px;color:var(--cyan,#00d4ff);font-weight:700;margin-bottom:6px">📈 ' + this._esc(country) + ' · 窗口逐日热度</div>' +
      '<div style="display:flex;align-items:flex-end;gap:3px;height:44px">';
    labels.forEach(function (l) {
      var v = buckets[l.k];
      var h = Math.max(3, Math.round(v / max * 40));
      html += '<span style="flex:1;border-radius:2px 2px 0 0;background:' + (v >= max * 0.66 && v > 0 ? 'linear-gradient(180deg,#ef4444,#ef444455)' : 'linear-gradient(180deg,#00d4ff,#0066ff55)') + ';height:' + h + 'px' + (v ? '' : ';opacity:.2') + '" title="' + l.lab + '：' + v + ' 条"></span>';
    });
    html += '</div><div style="display:flex;justify-content:space-between;font-size:9.5px;color:var(--text3,#7a8aa3);margin-top:2px"><span>' + labels[0].lab + '</span><span>峰值 ' + max + ' 条/日</span><span>' + labels[labels.length - 1].lab + '</span></div></div>';
    return html;
  },

  /* 关联情报：同国优先、同类其次（点击切换详情） */
  _relatedHTML: function (it, selfIdx) {
    var self = this;
    var rel = [];
    (this._items || []).forEach(function (x, xi) {
      if (xi === selfIdx || !x) return;
      var score = 0;
      if (it.country && x.country === it.country) score += 2;
      if (it.data_type && x.data_type === it.data_type) score += 1;
      if (score > 0) rel.push({ x: x, xi: xi, s: score });
    });
    rel.sort(function (a, b) { return b.s - a.s; });
    rel = rel.slice(0, 5);
    if (!rel.length) return '';
    var html = '<div style="margin-top:12px"><div style="font-size:10.5px;letter-spacing:2px;color:var(--cyan,#00d4ff);font-weight:700;margin-bottom:6px">🔗 关联情报（' + rel.length + ' · 点击切换）</div>';
    rel.forEach(function (r) {
      var lc2 = self._lvColor(r.x.level || '');
      html += '<div class="tr-item" style="margin-bottom:6px;border-left-color:' + lc2 + '" onclick="THREATROOM._closeDetail();THREATROOM._detail(' + r.xi + ')">' +
        '<div class="t" style="font-size:12px">' + self._hl(self._title(r.x), self._entity) + '</div>' +
        '<div class="m"><span>' + self._esc(r.x.source || '—') + '</span><span>' + self._when(r.x) + '</span>' +
        (r.x.country ? '<span>📍' + self._esc(r.x.country) + '</span>' : '') +
        (r.x.level ? '<span style="color:' + lc2 + '">' + self._lvCN(r.x.level) + '级</span>' : '') + '</div></div>';
    });
    return html + '</div>';
  },

  /* 复制情报要点 */
  _copyIntelNote: function (i) {
    var it = this._items[i];
    if (!it) return;
    var L = ['【专项情报要点】' + this._title(it)];
    L.push('信源：' + (it.source || '—') + '　时间：' + this._when(it) + '　类别：' + (this.DT_CN[it.data_type] || it.data_type || '—') + (it.country ? '　国别：' + it.country : '') + (it.level ? '　等级：' + this._lvCN(it.level) : ''));
    var desc = String(it.description || it.content || it.desc || it.summary || '').replace(/<[^>]+>/g, '').trim();
    if (desc) L.push('摘要：' + desc.slice(0, 400));
    if (it.url) L.push('原文：' + it.url);
    var done = function () { THREATROOM._toast('要点已复制到剪贴板'); };
    if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(L.join('\n')).then(done, done);
    else { var ta = document.createElement('textarea'); ta.value = L.join('\n'); document.body.appendChild(ta); ta.select(); try { document.execCommand('copy'); } catch (e) {} ta.remove(); done(); }
  },

  /* 转指挥事件（复用指挥中心 createIncidentFromAlert 口径，防重） */
  _toCommand: function (i) {
    var it = this._items[i];
    if (!it) return;
    if (typeof COMMAND === 'undefined' || !COMMAND.createIncidentFromAlert) { this._toast('指挥调度模块未加载'); return; }
    var now = new Date();
    var pad = function (x) { return (x < 10 ? '0' : '') + x; };
    var alertLike = {
      id: 'TR-CMD-' + String(it.id || (Date.now() + '-' + i)),
      title: this._title(it),
      level: it.level || 'yellow',
      ...
      desc: String(it.description || it.content || '').replace(/<[^>]+>/g, '').slice(0, 600)
    };
    try {
      var inc = COMMAND.createIncidentFromAlert(alertLike, {});
      if (inc) { this._toast('已转指挥事件：' + inc.id + '，可到指挥调度中心处置'); }
      else this._toast('转指挥失败：事件可能已存在');
    } catch (e) { this._toast('转指挥异常：' + e.message); }
  },

  /* ═══════════ 关联中资项目资产 → 项目复合详情（2026-09-05 用户指令二） ═══════════ */
  _projDetail: function (name) {
    var p = null;
    this._linkProjects().forEach(function (x) { if (x.n === name) p = x; });
    if (!p) { /* 兜底：直接从 ENTERPRISES 找 */
      (typeof ENTERPRISES !== 'undefined' ? ENTERPRISES : []).forEach(function (ent) {
        (ent.projects || []).forEach(function (pp) {
          if (pp.n === name) p = { n: pp.n, c: pp.c, ent: ent.short || ent.name, inv: pp.inv, p: pp.p };
        });
      });
    }
    if (!p) { this._toast('未找到项目档案：' + name); return; }
    var self = this;
    var st = this._stats();
    var heat = st.byCountry[p.c] || 0;
    /* 所在国情报（本次作战流） */
    var countryItems = (this._items || []).map(function (x, xi) { return { x: x, xi: xi }; })
      .filter(function (r) { return r.x && r.x.country === p.c; });
    /* 库内该国 24h 预警（全局 ALERTS） */
    var libAlerts = [];
    if (typeof ALERTS !== 'undefined') {
      var now = Date.now();
      ALERTS.forEach(function (a) {
        if (!a || a.country !== p.c) return;
        var t = new Date(a.time || a.date || 0).getTime();
        if (t && now - t <= 24 * 3600 * 1000) libAlerts.push(a);
      });
      libAlerts.sort(function (a, b) { return String(b.time || '').localeCompare(String(a.time || '')); });
    }
    var heatCol = heat >= 5 ? 'var(--red,#ef4444)' : heat ? 'var(--orange,#f59e0b)' : 'var(--green,#22c55e)';
    var html = '<div class="tr-modal" id="tr-modal" onclick="if(event.target===this)THREATROOM._closeDetail()"><div class="tr-modalbox" style="max-width:820px">';
    /* ① 项目档案卡 */
    html += '<div style="display:flex;align-items:flex-start;gap:10px">';
    html += '<div style="flex:1"><div style="font-size:16px;font-weight:800;color:var(--text,#e8eefc)">🏗 ' + this._esc(p.n) + '</div>' +
      '<div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:9px;font-size:11px">' +
      '<span class="tr-badge" style="border-color:var(--border2,#1e2a40)">承建 ' + this._esc(p.ent || '—') + '</span>' +
      '<span class="tr-badge" style="border-color:var(--cyan,#00d4ff)55;color:var(--cyan,#00d4ff)">📍' + this._esc(p.c || '—') + '</span>' +
      (p.inv ? '<span class="tr-badge" style="border-color:var(--border2,#1e2a40)">投资 ' + this._esc(String(p.inv)) + ' 亿</span>' : '') +
      (p.p ? '<span class="tr-badge" style="border-color:var(--border2,#1e2a40)">人员 ' + this._esc(String(p.p)) + '</span>' : '') +
      '<span class="tr-badge" style="border-color:' + heatCol + '55;color:' + heatCol + '">所在国窗口预警 ' + heat + ' 条</span>' +
      (libAlerts.length ? '<span class="tr-badge" style="border-color:var(--orange,#f59e0b)55;color:var(--orange,#f59e0b)">库内 24h 预警 ' + libAlerts.length + '</span>' : '') +
      '</div></div>';
    html += '<button onclick="THREATROOM._closeDetail()" style="background:none;border:1px solid var(--border2,#1e2a40);color:var(--text2,#aab8d0);border-radius:6px;padding:4px 12px;cursor:pointer;flex-shrink:0">✕ 关闭</button></div>';
    /* ② 动作区 */
    html += '<div style="display:flex;gap:9px;flex-wrap:wrap;margin-top:12px;border-top:1px solid var(--border2,#1e2a40);padding-top:11px">' +
      '<button onclick="THREATROOM._closeDetail();THREATROOM._flyTo(\'' + this._esc(p.c).replace(/'/g, '') + '\')" style="background:rgba(0,212,255,0.10);border:1px solid var(--cyan,#00d4ff);color:var(--cyan,#00d4ff);border-radius:6px;padding:5px 13px;cursor:pointer;font-size:12px">🗺 地图定位所在国</button>' +
      '<button onclick="THREATROOM._closeDetail();var _i=document.getElementById(\'tr-q\');if(_i)_i.value=\'' + this._esc(p.n).replace(/'/g, '') + '\';THREATROOM.run(\'' + this._esc(p.n).replace(/'/g, '') + '\')" style="background:rgba(0,212,255,0.06);border:1px solid var(--border2,#1e2a40);color:var(--text2,#aab8d0);border-radius:6px;padding:5px 13px;cursor:pointer;font-size:12px">🎯 对项目发起专项作战</button>' +
      '<button onclick="THREATROOM._copyProjNote(\'' + this._esc(p.n).replace(/'/g, '') + '\',\'' + this._esc(p.c).replace(/'/g, '') + '\',' + heat + ')" style="background:rgba(0,212,255,0.06);border:1px solid var(--border2,#1e2a40);color:var(--text2,#aab8d0);border-radius:6px;padding:5px 13px;cursor:pointer;font-size:12px">📋 复制防护要点</button></div>';
    /* ③ 该国时间线 */
    if (countryItems.length || heat) html += this._countryTlineHTML(p.c, -1);
    /* ④ 本次作战流·该国情报 */
    html += '<div style="margin-top:14px"><div style="font-size:10.5px;letter-spacing:2px;color:var(--cyan,#00d4ff);font-weight:700;margin-bottom:6px">📡 本次作战流 · ' + this._esc(p.c) + '（' + countryItems.length + ' 条 · 点击查看详情）</div>';
    if (!countryItems.length) html += '<div style="font-size:11.5px;color:var(--text3,#7a8aa3);padding:4px 0">本次作战流中无该国条目</div>';
    countryItems.slice(0, 8).forEach(function (r) {
      var lc2 = self._lvColor(r.x.level || '');
      html += '<div class="tr-item" style="margin-bottom:6px;border-left-color:' + lc2 + '" onclick="THREATROOM._closeDetail();THREATROOM._detail(' + r.xi + ')">' +
        '<div class="t" style="font-size:12px">' + self._hl(self._title(r.x), self._entity) + '</div>' +
        '<div class="m"><span>' + self._esc(r.x.source || '—') + '</span><span>' + self._when(r.x) + '</span>' +
        (r.x.level ? '<span style="color:' + lc2 + '">' + self._lvCN(r.x.level) + '级</span>' : '') +
        (r.x.chinaRelated ? '<span style="color:var(--orange,#f59e0b)">涉华</span>' : '') + '</div></div>';
    });
    html += '</div>';
    /* ⑤ 库内 24h 预警 */
    html += '<div style="margin-top:12px"><div style="font-size:10.5px;letter-spacing:2px;color:var(--orange,#f59e0b);font-weight:700;margin-bottom:6px">🚨 库内 24h 预警 · ' + this._esc(p.c) + '（' + libAlerts.length + ' 条 · 点击进预警中心详情）</div>';
    if (!libAlerts.length) html += '<div style="font-size:11.5px;color:var(--text3,#7a8aa3);padding:4px 0">库内 24h 无该国预警（安全窗口）</div>';
    libAlerts.slice(0, 8).forEach(function (a) {
      var lc3 = self._lvColor(a.level || '');
      html += '<div class="tr-item" style="margin-bottom:6px;border-left-color:' + lc3 + '" onclick="THREATROOM._closeDetail();if(typeof showAlertDetail===\'function\')showAlertDetail(\'' + String(a.id || '').replace(/'/g, '') + '\')">' +
        '<div class="t" style="font-size:12px">' + self._esc(a.title_zh || a.title || '（无标题）') + '</div>' +
        '<div class="m"><span>' + self._esc(a.source || '—') + '</span><span>' + String(a.time || '').slice(5, 16) + '</span>' +
        (a.level ? '<span style="color:' + lc3 + '">' + self._lvCN(a.level) + '级</span>' : '') +
        (a.chinaRelated ? '<span style="color:var(--orange,#f59e0b)">涉华</span>' : '') +
        (a.is_core ? '<span style="color:var(--red,#ef4444)">★核心</span>' : '') + '</div></div>';
    });
    html += '</div>';
    html += '</div></div>';
    var old = document.getElementById('tr-modal');
  ...
  /* 项目防护要点复制 */
  _copyProjNote: function (name, country, heat) {
    var L = ['【项目防护要点】' + name + '（' + country + '，' + new Date().toLocaleString('sv-SE').slice(0, 16) + '）'];
    L.push('所在国窗口预警 ' + heat + ' 条（专项作战室口径，近 ' + (this._days || 7) + ' 天）。');
    var countryItems = (this._items || []).filter(function (x) { return x && x.country === country; }).slice(0, 6);
    if (countryItems.length) {
      L.push('—— 所在国代表性情报 ——');
      countryItems.forEach(function (x, i) { L.push((i + 1) + '. ' + THREATROOM._title(x)); });
    }
    L.push('建议：核实现场人员与营地安防状态，视情升级安保等级并通报使领馆。');
    var done = function () { THREATROOM._toast('防护要点已复制'); };
    if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(L.join('\n')).then(done, done);
    else { var ta = document.createElement('textarea'); ta.value = L.join('\n'); document.body.appendChild(ta); ta.select(); try { document.execCommand('copy'); } catch (e) {} ta.remove(); done(); }
  },
