  ...
  var MA = {
    ...
    _renderNav: function () {
      ...
      host.innerHTML =
        ...
        '<div class="ma-nav-group">公文输出</div>' +
        '<div id="ma-govdoc" class="ma-nav-item" style="cursor:pointer" title="基于六专项模型异动信号与中枢研判，生成公文《风险预测模型专报》">' +
          '<span class="ma-ic">📄</span><span>风险预测模型专报</span></div>';
      host.querySelectorAll('[data-mv]').forEach(function (el) {
        el.addEventListener('click', function () { self._show(el.getAttribute('data-mv')); });
      });
      var gd = document.getElementById('ma-govdoc');
      if (gd) gd.addEventListener('click', function () { if (typeof GOVDOC !== 'undefined') GOVDOC.open('models'); });
