...
(function(){
  ...
  window.COMMAND = {
      ...
      var html = '<div class="command-center">' +
        '<div class="cmd-header">' +
          ...
          '<div class="cmd-actions">' +
            ...
            '<button class="btn sm" style="background:linear-gradient(135deg,var(--red),#b91c1c);color:#fff;border:none" onclick="COMMAND._startJointResponse()">🚀 启动联合响应</button>' +
            '<button class="btn sm" onclick="COMMAND.render()">🔄 刷新</button>' +
            '<button class="btn sm" onclick="COMMAND.showAuditLog()">📝 审计日志</button>' +
        ...
        '<div style="display:grid;grid-template-columns:3fr 2fr;gap:12px;margin-bottom:12px">' +
          '<div class="card" style="margin:0;border:1px solid rgba(255,51,85,0.25)"><div class="card-tt" style="font-size:12px"><span class="ic">🚨</span>实时预警接入 <span style="font-size:10px;color:var(--text3);font-weight:400">红色自动接警 · 橙色限量自动接入 · 其余可一键转</span></div>' +
            '<div style="max-height:240px;overflow-y:auto">' + this._renderAlertIntake() + '</div></div>' +
          '<div class="card" style="margin:0"><div class="card-tt" style="font-size:12px"><span class="ic">🏛️</span>部门协同负载 <span style="font-size:10px;color:var(--text3);font-weight:400">各部门在办工单</span></div>' +
            '<div style="max-height:240px;overflow-y:auto">' + this._renderDeptLoad() + '</div></div>' +
        '</div>' +
        /* ===== 第二行：指挥事件看板（三列） ===== */
        '<div class="card" style="margin:0 0 12px 0"><div class="card-tt" style="font-size:12px"><span class="ic">📋</span>指挥事件看板</div>' +
          '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">' +
            this._renderKanbanCol('待处理', pending, 'var(--yellow)') +
            this._renderKanbanCol('处置中', processing, 'var(--cyan)') +
            this._renderKanbanCol('已结案', closed.slice(0, 10), 'var(--green)') +
          '</div></div>' +
        /* ===== 第三行：资源调度 + 应急联络 ===== */
        '<div style="display:grid;grid-template-columns:3fr 2fr;gap:12px;margin-bottom:12px">' +
          '<div class="card" style="margin:0"><div class="card-tt" style="font-size:12px"><span class="ic">🚁</span>资源调度台 <span style="font-size:10px;color:var(--text3);font-weight:400">调度至最高等级待处理事件</span></div>' +
            this._renderResourceDesk() + '</div>' +
          '<div class="card" style="margin:0"><div class="card-tt" style="font-size:12px"><span class="ic">📞</span>应急联络</div>' +
            this._renderContacts() + '</div>' +
        '</div>' +
        /* ===== 全量事件列表 ===== */
        '<div class="card cmd-table-panel">' +
          '<div class="card-tt"><span class="ic">🗃️</span> 全部指挥事件</div>' +
          ...
          '</tr></thead><tbody>' +
          (this._incidents.length ? this._incidents.map(i => this._renderIncidentRow(i)).join('') : this._renderEmptyRow()) +
          '</tbody></table></div>' +
        '</div>' +
      '</div>';
      return html;
    ...
    _renderDeptLoad(){
      ...
      if(!keys.length) return '<div class="cmd-empty" style="padding:16px">暂无在办工单</div>';
      ...
      return keys.map(function(d){
        ...
        return '<div style="display:flex;align-items:center;gap:8px;padding:6px 10px">' +
          ...
          '<div style="flex:1;height:12px;background:var(--bg);border-radius:3px;overflow:hidden"><div style="width:' + pct + '%;height:100%;background:linear-gradient(90deg,rgba(0,212,255,0.4),rgba(0,212,255,0.85));border-radius:3px"></div></div>' +
          '<span style="width:28px;font-size:11px;font-weight:700;color:var(--cyan);text-align:right">' + load[d] + '</span></div>';
      }).join('');
    },

    /* 看板列（每列最多展示 12 条，超出折叠计数，避免自动接警后版面失控） */
    _renderKanbanCol(title, list, color){
      var MAX_SHOW = 12;
      var shown = list.slice(0, MAX_SHOW);
      var html = '<div style="background:var(--bg2);border:1px solid var(--border);border-top:2px solid ' + color + ';border-radius:8px;min-height:120px">' +
        '<div style="padding:8px 10px;font-size:11px;font-weight:700;color:' + color + ';border-bottom:1px solid var(--border)">' + title + ' <span style="color:var(--text3);font-weight:400">(' + list.length + ')</span></div>' +
        '<div style="padding:6px;display:flex;flex-direction:column;gap:6px;max-height:280px;overflow-y:auto">';
      if(!list.length){
        html += '<div style="text-align:center;color:var(--text3);font-size:11px;padding:18px 0">暂无事件</div>';
      }
      shown.forEach(function(i){
        var lv = RESPONSE_LEVELS.find(function(r){ return r.level === i.level; }) || RESPONSE_LEVELS[2];
        html += '<div style="background:var(--panel2);border:1px solid var(--border);border-left:3px solid ' + lv.color + ';border-radius:6px;padding:8px;cursor:pointer" onclick="COMMAND.openIncidentDetail(\'' + i.id + '\')">' +
          '<div style="font-size:11px;font-weight:600;color:var(--text1);line-height:1.4">' + _esc(i.title) + '</div>' +
          '<div style="display:flex;align-items:center;gap:6px;margin-top:4px;flex-wrap:wrap">' +
            ...
            '<span style="font-size:9px;color:var(--text3)">' + (i.country || '—') + '</span>' +
            '<span style="font-size:9px;color:var(--text3);margin-left:auto">📋' + i.workorderIds.length + '单</span>' +
          '</div></div>';
      });
    ...
    /* 资源调度台：驱动自资源库 this._resources（可增删改），实时反映派出状态 */
    _renderResourceDesk(){
      ...
      var me = this;
      var html = '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;padding:4px">' +
        res.map(function(r){
          ...
          return '<div style="padding:10px;background:var(--bg2);border:1px solid var(--border);border-radius:8px">' +
            ...
            '<div style="font-size:9px;color:var(--text3);margin-bottom:2px">' + _esc(r.contact || '') + '</div>' +
            '<div style="font-size:9px;color:' + statusColor + ';margin-bottom:6px">● ' + statusTxt + '</div>' +
            '<button class="btn sm" style="font-size:10px;width:100%" onclick="COMMAND._dispatchToActive(\'' + r.type + '\')">⚡ 调度</button>' +
          '</div>';
        }).join('') + '</div>';
    ...
    _renderContacts(){
      ...
      this._contacts.forEach(function(c){
        html += '<div style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:var(--bg2);border:1px solid var(--border);border-radius:6px">' +
        ...
        '<div class="cmd-modal-bd">' +
          '<div class="cmd-section">' +
            ...
            '<div class="cmd-info-grid">' +
              '<div class="cmd-info-item"><div class="lb">事件标题</div><div class="vl">' + _esc(inc.title) + '</div></div>' +
              ...
              '<div class="cmd-info-item"><div class="lb">负责部门</div><div class="vl">' + (inc.assignedDepts || []).map(d => DEPT_NAMES[d] || d).join('、') + '</div></div>' +
              '<div class="cmd-info-item"><div class="lb">工单进度</div><div class="vl">' + stat.done + ' / ' + stat.total + ' 已完成</div></div>' +
            '</div>' +
            '<div class="cmd-desc mt-12">' + (inc.desc ? _esc(inc.desc) : '暂无描述') + '</div>' +
    ...
    _renderMatchedPlaybooks(inc){
      ...
      inc.playbookIds.forEach(pid => {
        ...
        if(!p) return;
        html += '<div class="cmd-playbook-card">' +
          '<h5>' + _esc(p.name) + '</h5>' +
          '<div class="cmd-playbook-meta">' + _esc(p.category) + ' · 责任部门：' + p.dept.map(d => DEPT_NAMES[d] || d).join('、') + '</div>' +
          '<ul class="cmd-playbook-list">' +
            '<li><span class="cmd-playbook-label">触发词</span><span>' + (p.trigger ? p.trigger.slice(0,12).join('、') + (p.trigger.length>12?'…':'') : '—') + '</span></li>' +
    ...
    showPlaybookLibrary(){
      ...
      this._playbooks.forEach(p => {
        html += '<div class="cmd-playbook-card">' +
          ...
          '<div class="cmd-playbook-meta">' + _esc(p.category) + ' · 责任部门：' + p.dept.map(d => DEPT_NAMES[d] || d).join('、') + '</div>' +
          '<ul class="cmd-playbook-list">' +
            '<li><span class="cmd-playbook-label">触发词</span><span>' + (p.trigger || []).join('、') + '</span></li>' +
            '<li><span class="cmd-playbook-label">步骤</span><span>' + p.steps.length + ' 步</span></li>' +
            '<li><span class="cmd-playbook-label">联系人</span><span>' + (p.contacts || []).join('、') + '</span></li>' +
            '<li><span class="cmd-playbook-label">资源</span><span>' + (p.resources || []).join('、') + '</span></li>' +
          '</ul>' +
        '</div>';
